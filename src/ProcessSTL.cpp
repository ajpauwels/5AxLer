...
void ProcessSTL::addMeshFace(shared_ptr<MeshFace> p_face) {
    ...
    for (unsigned int i = 0; i < 3; ++i) {
        ...
        shared_ptr<MeshEdge> p_hashedEdge = emplacePair.first->first;
        shared_ptr<MeshFace> p_hashedFace = emplacePair.first->second;
        
        // Get whether the MeshFace was added
        bool faceWasAdded = emplacePair.second;
        
        printf("e1v1 = %s, e1v2 = %s\n", p_hashedEdge->p_vertex(0)->vertex().toString().c_str(), p_hashedEdge->p_vertex(1)->vertex().toString().c_str());
        ...
        if (!faceWasAdded) {
            ...
            int16_t hashedFaceEdgeIndex = p_hashedFace->getEdgeIndex(p_hashedEdge);
            ...
            p_hashedFace->connect(p_face, p_hashedFace->getEdgeIndex(p_hashedEdge));
            
            p_face->connect(p_hashedFace, i);
    ...
    if (getFileHandler(file)) {                        // Check that we opened successfully
        ...
        for (unsigned int i = 0; i < size; ++i) {		// Loop through all triangles
            ...
            for (unsigned int j = 0; j < 12; ++j) {		// Get all points from file
                ...
                points[j] = floor((points[j] * 1000) + 0.5);    //TODO the 1000 should be defined as a variable to change easily
            }
